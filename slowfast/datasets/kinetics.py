...
Teacher Detection
'''
from slowfast.teacher_detection.tld import TeacherDetection
import cv2
...
class Kinetics(torch.utils.data.Dataset):
    ...
    def _init_teacher_detector(self):
        # 在worker进程中初始化YOLO模型
        if self.teacher_detector is None:
            self.teacher_detector = TeacherDetection(self.cfg.MODEL.YOLO)

    def preprocess_tensor(self,frames, target_size=640):
        """
        将[num_frames, h, w, c] (uint8)的视频帧转换为 [num_frames, c, 640, 640] (float32)
        """
        num_frames, h, w, c = frames.shape
        output = torch.zeros(num_frames, c, 640, 640, dtype=torch.float32)
        ...
        for i in range(num_frames):
            ...
            # 3. 创建填充背景并居中放置
            padded = np.full((640, 640, c), 114, dtype=np.uint8)  # 灰色填充
            y_offset = (640 - new_h) // 2
            x_offset = (640 - new_w) // 2
            padded[y_offset:y_offset + new_h, x_offset:x_offset + new_w] = resized

            # 4. 转换回Tensor并调整维度
            tensor = torch.from_numpy(padded).permute(2, 0, 1).to(torch.uint8)  # [c, 640, 640]
            output[i] = tensor
    ...
    def teacher_detect(self, img):
        '''
        Args:
            imgs: [num_frames, c, 640, 640] (float32)

        Returns:transformed img with same size as inputs
        '''

        processed_array = self.teacher_detector(img / 255.0)

        return processed_array.permute(0,2,3,1).to(torch.uint8)
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            ...
            frames_decoded[0] = self.preprocess_tensor(frames_decoded[0])
            frames_decoded[0] = self.teacher_detect(frames_decoded[0])
